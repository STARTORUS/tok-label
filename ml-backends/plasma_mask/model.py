...
class LCFSModel(LabelStudioMLBase):
    ...
    def predict(self, tasks: List[Dict], context: Optional[Dict] = None, **kwargs) -> ModelResponse:
        ...
        if len(param) < 5:
            return ModelResponse(predictions=[])
        mask_l, mask_r = maskutils.generate_plasma_mask(**param, env=env_camera, angle=(0.5969, 2.7960))
        center_l, center_r = maskutils.generate_plasma_center(param['center'], env=env_camera)
        Pred_list = []
        Pred_list.append(prediction.BrushMask(maskutils.mask_to_rle(mask_l),'left','plasma'))
        Pred_list.append(prediction.BrushMask(maskutils.mask_to_rle(mask_r),'right','plasma'))
        ...
        pred_res = prediction.to_labelstudio_form(Pred_list, model_version='plasma_mask_script')
